...
const UNDO_ELEMENT = document.getElementById('undoElement')
const CHECK_ELEMENT = document.getElementById('checkElement')
const STAT_SCROLLER = document.getElementById('statScroller')
const PLAYERS = document.getElementById('meta_players').getAttribute('content')
const LOW_HEALTH = 10
const HIGH_CMDR_DMG = 15
...
for (const crownElement of document.getElementsByClassName('crown')) {
  crownElement.addEventListener('click', crownClick)
}

for (const nameField of document.getElementsByClassName('nameField')) {
  nameField.addEventListener('input', validateFields)
}

for (const playerForm of document.getElementsByClassName('playerForm')) {
...
function valueChange(element, diff) {
  if (diff == 0) return
  lowHealth(element)
  const amount = Math.abs(diff)
  const player = element.getAttribute('player')
  const commander = element.getAttribute('commander')
  if (commander != null) {
    highCmdrDmg(element)
    const lifeElement = document.querySelector(`.life[player="${player}"]`)
    lifeElement.innerHTML = Number(lifeElement.innerHTML) - diff
    STAT_SCROLLER.addStat(player, amount, '', commander)
    pushUndo([element, lifeElement], [Number(element.innerHTML) - diff, Number(lifeElement.innerHTML) + diff])
...
function popUndo() {
  ...
  if (undoObject) {
    for (let i = 0; i < undoObject.elements.length; i++) {
      const undoElement = undoObject.elements[i]
    ...
    if (undoHistory.length === 0) UNDO_ELEMENT.style.display = 'none'
    STAT_SCROLLER.removeStat()
  }
}

function lowHealth(lifeElement) {
  const health = Number(lifeElement.innerHTML)
  if (health <= LOW_HEALTH) lifeElement.setAttribute('lowHealth', 'true')
  else lifeElement.removeAttribute('lowHealth')
}

function highCmdrDmg(cmdrDmgElement) {
  const cmdrDmg = Number(cmdrDmgElement.innerHTML)
...
function endGame() {
  if (endGameTriggered === true) return
  endGameTriggered = true

  const endGameCurtain = document.getElementById('curtain')
  endGameCurtain.style.display = ''
  for (const form of document.getElementsByClassName('playerForm')) {
    form.style.display = ''
  }
  ...
  function submit(e) {
    ...
    for (let i = 0; i < PLAYERS; i++) {
      const nameField = document.querySelector(`.nameField[player='${i}']`)
...
function crownClick(e) {
  ...
  if (crownElement.getAttribute('value')) {
    ...
    for (const otherCrown of document.getElementsByClassName('crown')) {
